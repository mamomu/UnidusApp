...
export default function EventDetailsModal({
  ...
  const { topLevel: topLevelComments, replies } = organizeComments(
    (comments as Comment[]) || []
  );
  ...
  const handleAddParticipant = async (userId: number, permission: string) => {
    try {
      const res = await fetch(`/api/events/${event.id}/participants`, {
        method: "POST",
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    <Avatar className="w-8 h-8 bg-primary text-white">
                      <AvatarFallback>
                        {participants.owner.fullName.substring(0, 2)}
                      </AvatarFallback>
                    </Avatar>
                    <span className="ml-2 font-medium">
                      {participants.owner.fullName}
                    </span>
                  ...
                  >
                    <div className="flex items-center">
                      <Avatar className="w-8 h-8 bg-secondary text-white">
                        <AvatarFallback>
                          {participant.user.fullName.substring(0, 2)}
                        </AvatarFallback>
                      </Avatar>
                      <span className="ml-2 font-medium">
                    ...
                    topLevelComments.map((comment: Comment) => (
                      <div key={comment.id} className="flex">
                        <Avatar className="w-8 h-8 bg-primary text-white flex-shrink-0">
                          <AvatarFallback>
                            {comment.user?.fullName?.substring(0, 2) || "U"}
                          </AvatarFallback>
                        </Avatar>
                        <div className="ml-3 bg-neutral-100 p-3 rounded-lg rounded-tl-none flex-1">
                          ...
                          <div className="flex items-center mt-2 text-xs text-neutral-500">
                            <Button
                              variant="ghost"
                              size="sm"
                              className="p-0 h-auto text-xs text-neutral-500 hover:text-neutral-700"
                            >
                              Reply
                            </Button>
                    ...
                    <div className="flex justify-between items-center p-2 bg-neutral-50 border-t border-neutral-200">
                      <div className="flex items-center space-x-3">
                        <Button
                          ...
                          variant="ghost"
                          size="icon"
                          className="p-1.5 h-auto w-auto text-neutral-500"
                        >
